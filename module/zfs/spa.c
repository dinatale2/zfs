...
spa_activity_check_required(spa_t *spa, uberblock_t *ub, nvlist_t *config)
{
	...
	/*
	 * Skip the test if host which last imported the pool had MMP disabled
	 * or was using a non-MMP aware version.  If the pool is successfully
	 * imported, we will then respect the local setting for mmp writes.
	 */
	if ((ub->ub_mmp_magic == MMP_MAGIC && ub->ub_mmp_delay == 0) ||
	    (ub->ub_mmp_magic == 0 && ub->ub_mmp_delay == 0))
		return (B_FALSE);
	/*
    ...
    char **ereport)
{
	...
	/*
	 * For pools which have the safeimport property on determine if the
