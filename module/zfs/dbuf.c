...
typedef struct dbuf_stats {
	kstat_named_t dbuf_cache_count;
	...
	kstat_named_t dbuf_cache_levels[DN_MAX_LEVELS];
	kstat_named_t dbuf_cache_levels_bytes[DN_MAX_LEVELS];
	...
	kstat_named_t hash_dbuf_levels[DN_MAX_LEVELS];
	kstat_named_t hash_dbuf_levels_bytes[DN_MAX_LEVELS];
} dbuf_stats_t;

dbuf_stats_t dbuf_stats = {
	{ "dbuf_cache_count",			KSTAT_DATA_UINT64 },
...
dbuf_evict_one(void)
{
	...
	if (db != NULL) {
		...
		DBUF_STAT_BUMPDOWN(dbuf_cache_count);
	...
	if (holds == 0) {
		if (db->db_blkid == DMU_BONUS_BLKID) {
			...
			if (!DBUF_IS_CACHEABLE(db) ||
				...
				DBUF_STAT_BUMP(dbuf_cache_count);
				DBUF_STAT_INCR(
