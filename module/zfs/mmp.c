 ...
 * functionality is enabled by setting the safeimport property to on.
 *
 ...
 * second.  In order to assure the accuracy of the activity test default
 * values 10x were selected.
 *
 * The "zpool import"  activity test can be expected to take a minimum time
 * of zfs_mmp_import_intervals * zfs_mmp_interval milliseconds.  If the "best"
 * uberblock has a valid ub_mmp_delay field, then the duration of the test
 * may take longer if MMP writes were occuring less frequently than expected.
 * Additionally, the duration is then extended by a random 25% to attempt to
 * to detect simultaneous imports.  For example, if both partner hosts are
 * rebooted at the same time and automatically attempt to import the pool.
 */

/*
 * Used to control the frequency of mmp writes which are performed when the
 * safeimport pool property is on.  This is one  factor used to determine the
 * length of the activity check during import.
 *
 * The mmp write period is zfs_mmp_interval / leaf-vdevs milliseconds.  This
 * means that on average an mmp write will be issued for each leaf vdev every
 * zfs_mmp_interval milliseconds.  In practice, the observed period can vary
 * with the I/O load and this observed value is the delay which is stored in
 * the uberblock.  The minimum allowed value  is 100 ms.
 */
ulong_t zfs_mmp_interval = MMP_DEFAULT_INTERVAL;

/*
 * Used to control the duration of the activity test on import.  Smaller
 * values of zfs_mmp_import_intervals will reduce the import time but increase
 * the risk of failing to detect an active pool.  The total activity check
 * time is never allowed to drop below one second.  A value of 0 is ignored
 * and treated as if it was set to 1.
 */
 ...
 *
 * When zfs_mmp_fail_intervals = 0 then mmp write failures are ignored.
 * The failures will still be reported to the ZED which depending on its
 * configuration may take action such as suspending the pool or offlining a
 * device.
 *
 * When zfs_mmp_fail_intervals > 0 then sequential mmp write failures will
 * cause the pool to be suspended.  This occurs when zfs_mmp_fail_intervals *
 * zfs_mmp_interval milliseconds have passed since the last successful
 * mmp write.  This guarantees the activity test will see mmp writes if the
 * pool is imported.
 */
