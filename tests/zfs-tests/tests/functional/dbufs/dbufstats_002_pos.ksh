...
#
# NOTES:
# Currently, when dbufs are marked as DB_CACHED in the dbuf cache, the
# ARC header for the dbuf is not updated to reflect the "hit". Therefore,
# the only way to cause a dbuf to transition from mru to mfu is to ensure
# that it is in the ARC, but not in the dbuf cache.
#

DBUFS_FILE="/var/tmp/dbufs.out.$$"
DBUF_CACHE_SIZE=$(get_tunable dbuf_cache_max_bytes)

function cleanup
{
	log_must rm -f $TESTDIR/file $TESTDIR/file2 $DBUFS_FILE
	log_must set_tunable64 dbuf_cache_max_bytes "$DBUF_CACHE_SIZE"
}
...
log_must set_tunable64 dbuf_cache_max_bytes "$((5*1024*1024))"

log_must dd if=/dev/urandom bs=1M count=1 of="$TESTDIR/file"
log_must zpool sync
...
log_must dd if=/dev/urandom bs=1M count=50 of="$TESTDIR/file2"
log_must zpool sync
