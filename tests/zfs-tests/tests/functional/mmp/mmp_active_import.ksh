...
# DESCRIPTION:
#	Under no circumstances when safeimport is active, should an active pool
...
log_assert "safeimport=on|off active pool activity checks"
log_onexit cleanup

# 1. Simulate an active pool on another host with ztest.
mmp_pool_destroy $MMP_DIR $MMP_POOL
mmp_pool_create $MMP_DIR $MMP_POOL

# 2. Verify 'zpool import' reports an active pool.
log_must set_spl_tunable spl_hostid $SPL_HOSTID2
log_must set_tunable64 zfs_mmp_interval $MMP_INTERVAL_MIN
log_must is_pool_imported $MMP_DIR $MMP_POOL

# 3. Verify 'zpool import [-f] $MMP_POOL' cannot import the pool.
MMP_IMPORTED_MSG="Cannot import '$MMP_POOL': pool is imported"
log_must try_pool_import $MMP_DIR $MMP_POOL "" $MMP_IMPORTED_MSG
for i in {1..10}; do
	log_must pgrep ztest >/dev/null
	log_must try_pool_import $MMP_DIR $MMP_POOL "-f" $MMP_IMPORTED_MSG
done
...
log_must wait_pool_imported $MMP_DIR $MMP_POOL
...
log_must check_pool_import $MMP_DIR $MMP_POOL "action" $MMP_IMPORTED_MSG
...
log_must check_pool_import $MMP_DIR $MMP_POOL "status" $MMP_IMPORTED_MSG
...
# 7. Verify 'zpool import -f $MMP_POOL' can now import the pool.
log_must import_activity_check "-f -d $MMP_DIR" $MMP_POOL

# 8 Verify pool may be exported/imported without -f argument.
log_must zpool export $MMP_POOL
log_must import_no_activity_check "-d $MMP_DIR" $MMP_POOL

log_pass "safeimport=on|off active pool activity checks passed"
